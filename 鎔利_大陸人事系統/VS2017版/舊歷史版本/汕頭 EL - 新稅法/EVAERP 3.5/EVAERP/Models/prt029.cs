...
namespace EVAERP.Models
{
    class prt029
    {
        ...
        public decimal Vaca_l { get; set; }
        public decimal Vaca_m { get; set; }
        public decimal Vaca_n { get; set; }
        public decimal Vaca_o { get; set; }
        public decimal Va_spec_hour { get; set; }
        #endregion

        public static IEnumerable<prt029> ToDoList(decimal Va_year)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Va_year);
            sql = null;
            sql += "select * from prt029 where 1= 1 ";
            sql += " and va_year = ?";
            sql += " order by va_year,Tr_id_no ";
            ...
            return from p in DeptDS.Tables[0].AsEnumerable()
                   select new prt029
                   {
                       ...
                       Vaca_l = p.IsNull("vaca_l") ? 0 : p.Field<decimal>("vaca_l"),
                       Vaca_m = p.IsNull("vaca_m") ? 0 : p.Field<decimal>("vaca_m"),
                       Vaca_n = p.IsNull("vaca_n") ? 0 : p.Field<decimal>("vaca_n"),
                       Vaca_o = p.IsNull("vaca_o") ? 0 : p.Field<decimal>("vaca_o"),
        ...
        public string Insert()
        {
            try
            {
                ...
                parm.Add(string.IsNullOrEmpty(System.Convert.ToString(Vaca_l)) ? 0 : Vaca_l);
                parm.Add(string.IsNullOrEmpty(System.Convert.ToString(Vaca_m)) ? 0 : Vaca_m);
                parm.Add(string.IsNullOrEmpty(System.Convert.ToString(Vaca_n)) ? 0 : Vaca_n);
                parm.Add(string.IsNullOrEmpty(System.Convert.ToString(Vaca_o)) ? 0 : Vaca_o);
                ...
                {
                    string sql = null;
                    sql += "insert into prt029";
                    sql += "(va_year,va_pr_no,va_id_no,va_spec_date,va_spec_month,vaca_a,vaca_b,vaca_c,vaca_d,vaca_e,";
                    sql += "vaca_f,vaca_g,vaca_h,vaca_i,vaca_j,vaca_k,vaca_l,vaca_m,vaca_n,vaca_o,va_spec_hour)";
                    sql += " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        public string Delete(decimal Va_year, string Va_pr_no)
        {
            try
            {
                ...
                parm.Add(Va_pr_no);
                string sql = null;
                sql += "delete from prt029 where va_year=? and va_pr_no=?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
        ...
        public string Update(decimal Va_year, string Va_pr_no)
        {
            try
            {
                ...
                string sql = null;
                sql += "update prt029 set va_id_no=?,va_spec_date=?,va_spec_month=?,vaca_a=?,vaca_b=?,vaca_c=?,vaca_d=?,vaca_e=?,vaca_f=?,";
                sql += "vaca_g=?,vaca_h=?,vaca_i=?,vaca_j=?,vaca_k=?,vaca_l=?,vaca_m=?,vaca_n=?,vaca_o=?,va_spec_hour=? ";
                sql += " where va_year =?";
                sql += " and va_pr_no=?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "修改失敗";
        ...
        public static prt029 Get(decimal Va_year, string Va_pr_no)
        {
            // 查詢資料庫資料
            ArrayList parm = new ArrayList();
            parm.Add(Va_year);
            ...
            return new prt029
            {
                ...
                Vaca_l = DeptDS.Tables[0].Rows[0].IsNull("vaca_l") ? 0 : DeptDS.Tables[0].Rows[0].Field<decimal>("vaca_l"),
                Vaca_m = DeptDS.Tables[0].Rows[0].IsNull("vaca_m") ? 0 : DeptDS.Tables[0].Rows[0].Field<decimal>("vaca_m"),
                Vaca_n = DeptDS.Tables[0].Rows[0].IsNull("vaca_n") ? 0 : DeptDS.Tables[0].Rows[0].Field<decimal>("vaca_n"),
                Vaca_o = DeptDS.Tables[0].Rows[0].IsNull("vaca_o") ? 0 : DeptDS.Tables[0].Rows[0].Field<decimal>("vaca_o"),
