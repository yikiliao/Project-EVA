...
{
    public partial class qrr220 : Form
    {
        ...
        {
            f_yy.DataSource = prt037L.ToDoList_YY().ToList();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var count = 0;
            count = prt037L.ToDoList(YY, Dept, Cdept, Prno).Count();
            if (count == 0)
            {
                Config.Show("...無符合資料...");
        ...
        private void Print_cs(string Dept, string Cdept, Int16 YY, string Prno)
        {
            var rpname = "年終獎金所得稅申報表";

            CrystalReport_qrr010 rp = new CrystalReport_qrr010();
            rp.SetDataSource(prt037L.ToDoList(YY, Dept, Cdept, Prno));

            rp.SetParameterValue("CompName", sst011.Get().Company_name);//公司名稱            
            rp.SetParameterValue("ReportName", string.Format("{0}年度 {1}", YY, rpname));//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, YY,Prno));//列印條件
            rp.SetParameterValue("ReportId", "qrr220");//程式編號            
