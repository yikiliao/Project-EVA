...
{
    public partial class crr015 : Form
    {        
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (mcrr015.ToDoList(Dept, Cdept).Count() == 0)
            {
                Config.Show("...無符合資料...");
        ...
        {            
            CrystalReport_crr015 rp = new CrystalReport_crr015();
            rp.SetDataSource(mcrr015.ToDoList(Dept, Cdept));

            rp.SetParameterValue("CompName", sst011.Get().Company_name);//公司名稱
            rp.SetParameterValue("ReportName", "教育程度分析表");//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, Cdept_Detail));//列印條件
            rp.SetParameterValue("ReportId", "crr015");//程式編號
