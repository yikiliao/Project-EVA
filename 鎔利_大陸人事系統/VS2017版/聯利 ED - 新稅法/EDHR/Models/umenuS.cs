...
namespace EDHR.Models
{
    class umenuS
    {
        ...
        public static IEnumerable<umenuS> Treeview(string User_account)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(User_account.Trim());
            sql = null;
            sql += "select * from umenuS where 1=1 ";
        ...
        public static IEnumerable<umenuS> Select()
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            sql = null;
            sql += "select DISTINCT id,parentId,idname,explanation from umenuS where 1=1 and programs = 'Y'";
            sql += " order by id ";
            DataSet DeptDS = DBConnector.executeQuery(Conn.GetStr(Login.DB), sql, parm);
            return from q in DeptDS.Tables[0].AsEnumerable()
                   select new umenuS
        ...
        public static IEnumerable<umenuS> SelectByUser(string User)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(User.Trim());
        ...
        public static IEnumerable<umenuS> ToDoList(string UserId)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            sql = null;
            sql += "select DISTINCT user_account from umenuS where 1= 1 ";
        ...
        public string Insert()
        {
            try
            {
                ...
                if (umenuS.Get(Id, User_account) != null)
                {
                    return "已有此資料";
                }
        ...
        public string Delete(string UserId)
        {
            try
            {
                ...
                parm.Add(UserId);
                string sql = null;
                sql += "delete from umenuS where user_account=?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
        ...
        public static umenuS Get(string Id)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Id);
            sql = null;
            sql += "select idName from umenuS where id = ?";
            DataSet DeptDS = DBConnector.executeQuery(Conn.GetStr(Login.DB), sql, parm);
        ...
        public static umenuS Get(string Id, string UserId)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Id);
            parm.Add(UserId);
            sql = null;
            sql += "select idName from umenuS where id = ? and user_account=?";
            DataSet DeptDS = DBConnector.executeQuery(Conn.GetStr(Login.DB), sql, parm);
            if (DeptDS.Tables[0].Rows.Count == 0)
                return null;
            return new umenuS
