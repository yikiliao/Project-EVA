...
namespace EDHR.Models
{
    class prt035D
    {
        ...
        public static IEnumerable<prt035D> ToDoList_YY()
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            sql = null;
            sql += "select DISTINCT(yy) from prt035D ";
            sql += " order by 1 desc";
        ...
        public static IEnumerable<prt035D> ToDoList(decimal Yy, string Dept, string Cdept, string Pr_no)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            ...
            sql = null;
            sql += "select prt035D.*,prt016.pr_name from prt035D  ";
            sql += " LEFT OUTER JOIN prt016 on prt016.pr_no = prt035D.pr_no ";
            sql += " where 1= 1";
            sql += " and prt035D.yy =?";
            sql += " and prt035D.dept =?";
            //部門
            if (!string.IsNullOrEmpty(Cdept.Trim()))
            {
                sql += " and prt035D.cdept in " + String.Format("({0})", Cdept.Trim());
            }
            //工號
            if (!string.IsNullOrEmpty(Pr_no.Trim()))
            {
                sql += " and prt035D.pr_no in " + String.Format("({0})", Pr_no.Trim());
            }
            sql += " order by prt035D.yy,prt035D.dept,prt035D.cdept,prt035D.pr_no ";
        ...
        public static prt035D Get(decimal Yy, string Dept, string Cdept, string Pr_no)
        {
            // 查詢資料庫資料
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
        ...
        public string Delete(decimal Yy, string Dept)
        {
            try
            {
                ...
                string sql = null;
                sql += "delete from prt035D where 1=1 ";
                sql += " and yy =?";
                sql += " and dept =?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
        ...
        public static IEnumerable<prt035D> ToDoList_Cdept(decimal Yy, string Dept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Dept);
            sql = null;
            sql += "select DISTINCT(cdept),prt001.department_name_new  from prt035D ";
            sql += "LEFT OUTER JOIN prt001 on prt001.department_code = prt035D.cdept";
            sql += " where 1=1 ";
            sql += " and prt035D.yy=?";
            sql += " and prt035D.dept=?";
            sql += " order by prt035D.cdept";
        ...
        public static IEnumerable<prt035D> ToDoList_Prno(decimal Yy, string Dept, string Cdept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Dept);
            sql = null;
            sql += "select prt035D.pr_no pr_no,prt016.pr_name pr_name  from prt035D ";
            sql += "LEFT OUTER JOIN prt016 on prt016.pr_no = prt035D.pr_no ";
            sql += " where 1=1 ";
            sql += " and prt035D.yy=?";
            sql += " and prt035D.dept=?";
