...
namespace EDHR.Models
{
    class prt002
    {
        ...
        public static IEnumerable<prt002> ToDoList(string Code, string Code_desc,string Remark)
        {
            string sql = null;
            ArrayList parm = new ArrayList();          
            sql = null;
            sql += "select * from prt002 where 1=1 ";
            if (Code.Length != 0)
            {
        ...
        public string Delete(string Code)
        {
            try
            {
                ...
                parm.Add(Code);
                string sql = null;
                sql += "delete from prt002 where code=?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
        ...
        public string Update(string Code)
        {
            try
            {
                ...
                parm.Add(string.IsNullOrEmpty(Code) ? null : Code.Trim());
                string sql = null;
                sql += "update prt002 set code_desc=?,vaild=?,remark=?,mod_date=?,mod_user=? ";
                sql += " where code =?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "修改失敗";
            }
        ...
        public static prt002 Get(string Code)
        {
            // 查詢資料庫資料
            ArrayList parm = new ArrayList();
            parm.Add(Code);
        ...
        public static IEnumerable<prt002> ToDoListCode(string Vaild)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            sql = null;
            sql += "select code,code_desc  from prt002 where 1= 1 ";
            if (Vaild.Length > 0)
            {
                parm.Add(Vaild.Trim());
                sql += " and vaild= ?";
