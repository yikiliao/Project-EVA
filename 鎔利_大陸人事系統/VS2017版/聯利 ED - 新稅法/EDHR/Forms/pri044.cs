...
namespace EDHR.Forms
{
    public partial class pri044 : Form
    {
        ...
        private void f_Query()
        {
            ...
            LS1 = prt032.ToDoGroupList(Dept, _yy, _mm).ToList();
            if (LS1.Count() == 0)
            {
                Config.Show("無符合資料");
                return;
        ...
        private void f_show(int idx)
        {
            ...
            {
                f_dept.SelectedValue = LS1[i].Dept;
                f_yy.Text = LS1[i].Yy.ToString();
                f_mm.Text = LS1[i].Mm.ToString();
                //明細資料
                bindingSource1.Clear();
                bindingSource1.DataSource = prt032.ToDoList(Dept).Where(m => m.Yy == LS1[i].Yy && m.Mm == LS1[i].Mm).ToList();
            }
        }
