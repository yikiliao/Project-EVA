...
{
    public partial class crr040 : Form
    {   
        List<bool> LCdept = new List<bool>();//存 部門勾選

        List<mcrr040L> Lmcrr040L = new List<mcrr040L>(); //L 廠用
        List<mcrr040S> Lmcrr040S = new List<mcrr040S>(); //S 廠用
        public crr040()
        {
            InitializeComponent();            
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                Lmcrr040L.Clear();
                Lmcrr040L = mcrr040L.ToDoList(Cdept, YY, MM, Type).ToList();
                count = Lmcrr040L.Count();
            }
            else
            {
                Lmcrr040S.Clear();
                Lmcrr040S = mcrr040S.ToDoList(Cdept, YY, MM, Type).ToList();
                count = Lmcrr040S.Count();
            }
            ...
            else
            {
                ...
                {
                    case "L":
                        L_Print_cs(Dept, Cdept, YY, MM, Type, Lmcrr040L);
                        break;
                    case "S":
                        S_Print_cs(Dept, Cdept, YY, MM, Type, Lmcrr040S);
                        break;
                    default:
                        L_Print_cs(Dept, Cdept, YY, MM, Type, Lmcrr040L);
                        break;
        ...
        private void L_Print_cs(string Dept, string Cdept, Int16 YY, Int16 MM, string Type, List<mcrr040L> Lmcrr040L)
        {
            var rpname = "個人所得申報表";

            CrystalReport_crr040L rp = new CrystalReport_crr040L();
            rp.SetDataSource(Lmcrr040L);
            ...
            rp.SetParameterValue("ReportId", "crr040");//程式編號            
            crystalReportViewer1.ReportSource = rp;
            crystalReportViewer1.Refresh();
        }

        private void S_Print_cs(string Dept, string Cdept, Int16 YY, Int16 MM, string Type, List<mcrr040S> Lmcrr040S)
        {
            var rpname = "個人所得申報表";

            CrystalReport_crr040S rp = new CrystalReport_crr040S();
            rp.SetDataSource(Lmcrr040S);
            rp.SetParameterValue("CompName", sst011.Get().Company_name);//公司名稱
            rp.SetParameterValue("ReportName", string.Format("{0}年{1}月{2}", YY, MM, rpname));//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, YY, MM, Type));//列印條件
            rp.SetParameterValue("ReportId", "crr040");//程式編號            
