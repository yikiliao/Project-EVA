...
namespace EVAERP.Models
{
    class mcrr001
    {
        ...
        public string Pr_Id { get; set; }//身分證
        public DateTime Pr_Inday { get; set; }//入廠日
        public string Job_Name { get; set; } //職稱
        public int Pr_Live_Pr { get; set; }//扶養人數
        #endregion

         public static IEnumerable<mcrr001> ToDoList(string Dept, string Cdept)
        {
            string sql = null;
            ...
            return from p in DeptDS.Tables[0].AsEnumerable()
                   select new mcrr001
                   {
                       ...
                       Pr_Id = p.IsNull("pr_idno") ? "" : p.Field<string>("pr_idno").Trim(),
                       ...
                       Pr_Live_Pr = p.IsNull("pr_live_pr") ? 0 : p.Field<int>("pr_live_pr"),
                   };
        }
         ...
         //               Cdept_Name = prt001.Get(p.Field<string>("cdept_name").Trim()) == null ? "" : prt001.Get(p.Field<string>("cdept_name").Trim()).Department_name_new,
         //               Pr_No = p.IsNull("pr_no") ? "" : p.Field<string>("pr_no").Trim(),
         //               Pr_Name = p.IsNull("pr_name") ? "" : p.Field<string>("pr_name").Trim(),
         //               Pr_Sex = p.IsNull("pr_sex") ? "" : p.Field<string>("pr_sex").Trim(),
         //               Pr_Birthday = System.Convert.ToDateTime(p.Field<string>("pr_birth_date").Trim()),
