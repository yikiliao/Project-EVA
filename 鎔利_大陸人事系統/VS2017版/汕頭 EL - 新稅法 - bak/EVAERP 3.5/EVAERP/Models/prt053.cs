...
namespace EVAERP.Models
{
    class prt053
    {
        ...
        public static IEnumerable<prt053> ToDoList(string Dept, string Tax_date)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            sql = null;
            sql += "select * from prt053 where 1= 1 ";
        ...
        public string Insert()
        {
            try
            {
                ...
                if (prt053.Get(Dept, Tax_date, Seq_no) != null)
                {
                    return "已有此筆資料";
                }
                else
        ...
        public string Delete(string Dept, string Tax_date, decimal Seq_no)
        {
            try
            {
                ...
                parm.Add(Seq_no);
                string sql = null;
                sql += "delete from prt053 where dept=? and tax_date=? and seq_no=?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
        ...
        public static IEnumerable<prt053> ToDoListByGroup(string Dept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            sql = null;
            sql += "select DISTINCT dept,tax_date from prt053 where 1= 1 ";
            if (!string.IsNullOrEmpty(Dept.Trim()))
            {
                parm.Add(Dept.Trim());
                sql += " and dept = ?";
        ...
        public static prt053 Get(string Dept, string Tax_date, decimal Seq_no)
        {
            // 查詢資料庫資料
            ArrayList parm = new ArrayList();
            parm.Add(Dept);
        ...
        public static prt053 Get(string Dept, decimal rAmt1)
        {
            // 查詢資料庫資料
            ArrayList parm = new ArrayList();
            parm.Add(Dept);
