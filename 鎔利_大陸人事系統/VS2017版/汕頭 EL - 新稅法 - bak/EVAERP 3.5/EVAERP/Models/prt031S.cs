    ...
    class prt031S
    {
        ...
        public string Delete(decimal Yy, decimal Mm)
        {
            try
            {
                ...
                parm.Add(Mm);
                string sql = null;
                sql += "delete from prt031S where yy=? and mm=? ";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
            ...
            decimal Tot_wtime, decimal Tot_vtime1, decimal Tot_vtime2, decimal Tot_ntime, decimal Tot_atime1, decimal Tot_atime2, decimal Tot_atime)
        {
            ...
            //所得稅
            p_prt031S.Amt_19 = Tax(Dept, (int)Yy, (int)Mm, Pr_no, p_prt031S.Amt_16, p_prt031S.Amt_17, p_prt031S.Amt_21, p_prt031S.Amt_22, p_prt031S.Amt_23, p_prt031S.Amt_26);
