...
{
    public partial class crr022 : Form
    {   
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (mcrr022.ToDoList(Dept, Cdept, Type).Count() == 0)
            {
                Config.Show("...無符合資料...");
        ...
        private void Print_cs(string Dept, string Cdept,string Type)
        {            
            CrystalReport_crr022 rp = new CrystalReport_crr022();
            rp.SetDataSource(mcrr022.ToDoList(Dept, Cdept, Type));

            rp.SetParameterValue("CompName", sst011.Get().Company_name);//公司名稱            
            rp.SetParameterValue("ReportName", "薪資基準表");//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, Type));//列印條件
            rp.SetParameterValue("ReportId", "crr022");//程式編號
        ...
        private string f_Cond(string Dept, string Cdept,string Type)
        {            
            string cond = "";            
