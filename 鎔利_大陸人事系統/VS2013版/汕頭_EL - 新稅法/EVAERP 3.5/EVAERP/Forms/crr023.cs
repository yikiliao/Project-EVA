...
namespace EVAERP.Forms
{
    ...
    {        
            ...
            else
            {
                switch (Dept)
                {
                    case "L":
                        if (OutType == "1")
                        {
                            L_Print_cs(Dept, Cdept, YY, MM, Type, Cdept_Detail, OutType, Prno);//A4
                        }
                        else
                        {
                            L_Print_xls(Dept, Cdept, YY, MM, Type, Cdept_Detail, OutType, Prno);//xls
                        }
                        break;
                    case "S":
                        if (OutType == "1")
                        {
                            S_Print_cs(Dept, Cdept, YY, MM, Type, Cdept_Detail, OutType, Prno);//A4
                        }
                        else
                        {
                            S_Print_xls(Dept, Cdept, YY, MM, Type, Cdept_Detail, OutType, Prno);//xls
                        }
                        break;
                    default:
                        if (OutType == "1")
        ...
        private void L_Print_cs(string Dept, string Cdept, Int16 YY, Int16 MM, string Type, string Cdept_Detail, string OutType, string Prno)
        {
            var rpname = Cdept_Detail == "Y" ? "薪資明細表" : "薪資彙總表";

            CrystalReport_crr023 rp = new CrystalReport_crr023();
        ...
        private void L_Print_xls(string Dept, string Cdept, Int16 YY, Int16 MM, string Type, string Cdept_Detail, string OutType, string Prno)
        {
            var rpname = Cdept_Detail == "Y" ? "薪資明細表" : "薪資彙總表";
            CrystalReport_crr023_xls rp = new CrystalReport_crr023_xls();
            rp.SetDataSource(mcrr023.ToDoList(Cdept, YY, MM, Type, Prno));
            rp.SetParameterValue("CompName", sst011.Get().Company_name);//公司名稱
            rp.SetParameterValue("ReportName", string.Format("{0}年{1}月{2}", YY, MM, rpname));//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, YY, MM, Type, Cdept_Detail, OutType, Prno));//列印條件
