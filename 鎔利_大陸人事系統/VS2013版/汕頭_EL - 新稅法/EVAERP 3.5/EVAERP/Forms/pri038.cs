...
namespace EVAERP.Forms
{
    public partial class pri038 : Form
    {
        ...
        private bool f_Check()
        {
            ...
            foreach (var it in prt023.ToDoList(""))
            {
        ...
        private string f_DDelete()
        {
            return String.Format("{0}", f_DDelete_prt023());
        }
        ...
        private string f_DDelete_prt023()
        {
            var p_prt023 = new prt023();            
            return p_prt023.Delete(f_pr_no.Text, System.Convert.ToInt32(f_cont_seq.Text));
        }
        ...
        {
            LS1.Clear();
            LS1 = prt023.ToDoList(rPrno,Dept,rCont).ToList();
            if (LS1.Count() == 0)
            {
                Config.Show("無符合資料");
                return;
        ...
        private void f_show(int idx)
        {
            ...
            {
                var q_prt023 = prt023.Get(LS1[idx].Pr_no, LS1[idx].Cont_seq);

                f_dept.SelectedValue = q_prt023.Dept;
                f_pr_no.Text = q_prt023.Pr_no;
                Find_p(f_pr_no.Text);
                f_cont_seq.Text = q_prt023.Cont_seq.ToString();
                f_cont_no.Text = q_prt023.Cont_no;
                f_cont_type.SelectedValue = q_prt023.Cont_type.ToString();
                f_beg_date.Text = q_prt023.Beg_date;
                f_end_date.Text = q_prt023.End_date;
                f_cont_year.Text = q_prt023.Cont_year.ToString();
                f_cont_month.Text = q_prt023.Cont_month.ToString();
                f_cont_not.Text = q_prt023.Cont_not.ToString();
                f_rem_no.Text = q_prt023.Rem_no;
                f_rem_code.SelectedValue = q_prt023.Rem_code.ToString();
                f_memo.Text = q_prt023.Memo;
                f_rem_date.Text = q_prt023.Rem_date;
                f_add_date.Text = q_prt023.Add_date.ToString();
                f_add_user.Text = q_prt023.Add_user.Trim();
                f_mod_date.Text = q_prt023.Mod_date.ToString();
                f_mod_user.Text = q_prt023.Mod_user.Trim();
        ...
        private void menu_del_Click_1(object sender, EventArgs e)
        {
            if (f_pr_no.Text != string.Empty)
            {
                //if (p_prt016.Pr_leave_date != string.Empty)
