...
{
    public partial class crr014 : Form
    {
        ...
        {   
            ...
            if (mcrr014.ToDoList(Dept, Cdept).Count() == 0)
            {
                Config.Show("...無符合資料...");
        ...
        {            
            CrystalReport_crr014 rp = new CrystalReport_crr014();
            rp.SetDataSource(mcrr014.ToDoList(Dept, Cdept));

            rp.SetParameterValue("CompName", sst011.Get(Login.DEPT).Company_name);//公司名稱            
            rp.SetParameterValue("ReportName", "年資分析表");//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, Cdept_Detail));//列印條件
            rp.SetParameterValue("ReportId", "crr014");//程式編號
