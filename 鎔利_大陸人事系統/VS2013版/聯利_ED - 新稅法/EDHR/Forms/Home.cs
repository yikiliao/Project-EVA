...
namespace EDHR.Forms
{
    public partial class Home : Form
    {
        ...
        static List<umenuD> LS1 = new List<umenuD>();
        ...
        public Home(string Userid)
        {
            ...
            this.Text += String.Format("         {0}  【登入使用者：{1}－{2}】",sst011.Get(Login.DEPT).Company_name, sst901.Get(Id).Pr_name.Trim(), Conn.GetName(Login.DB));
        }
        ...
        {
            LS1.Clear();
            LS1 = umenuD.Treeview(Id).ToList();//依使用者抓menu            
            if (LS1.Count() == 0)
            {
