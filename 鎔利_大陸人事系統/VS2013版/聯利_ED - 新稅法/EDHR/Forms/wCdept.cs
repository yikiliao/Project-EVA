...
namespace EDHR.Forms
{
    public partial class wCdept : Form
    {
        ...
        public wCdept(decimal Yy, string Dept, string SType)
        {
            InitializeComponent();
            Config.SetWindowSize(this);
            if (SType == "prt035D")
                bindingSource1.DataSource = prt035D.ToDoList_Cdept(Yy, Dept).ToList();
            if (SType == "prt036D")
                bindingSource1.DataSource = prt036D.ToDoList_Cdept(Yy, Dept).ToList();
            if (SType == "prt037D")
                bindingSource1.DataSource = prt037D.ToDoList_Cdept(Yy, Dept).ToList();
            Add_CheckBox();//加入checkbox的欄位在
        }
