...
{
    public partial class crr032 : Form
    {   
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string ReportWite = f_ckh_isWhiteReport.Checked == true ? "Y" : "N";
                        
            //列印資料
            var Org_Color = lb_msg.ForeColor;
            Cursor_wait(); //改變滑鼠游標漏斗指標
            if (mcrr032.ToDoList(Dept, Cdept,Beg_date,End_date).Count() == 0)
            {
                Config.Show("...無符合資料...");
            ...
            {
                Print_cs(Dept, Cdept, Beg_date, End_date, ReportWite);
            }
        ...
        private void Print_cs(string Dept, string Cdept, DateTime Beg_date, DateTime End_date,string ReportWite)
        {
            var ReportName = ReportWite == "Y" ? "出勤日報表-空白" : "出勤日報表";
            CrystalReport_crr032 rp = new CrystalReport_crr032();
            rp.SetDataSource(mcrr032.ToDoList(Dept, Cdept, Beg_date, End_date));
            ...
            rp.SetParameterValue("ReportId", "crr032");//程式編號 
