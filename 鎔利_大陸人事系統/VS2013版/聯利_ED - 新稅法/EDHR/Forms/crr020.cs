...
{
    public partial class crr020 : Form
    {   
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (mcrr020.ToDoList(YY, Dept, Cdept,"").Count() == 0)
            {
                Config.Show("...無符合資料...");
        ...
        {  
            CrystalReport_crr020 rp = new CrystalReport_crr020();
            rp.SetDataSource(mcrr020.ToDoList(YY, Dept, Cdept,""));

            rp.SetParameterValue("CompName", sst011.Get(Login.DEPT).Company_name);//公司名稱            
            rp.SetParameterValue("ReportName", string.Format("{0}年度員請假統計表表",YY ));//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, YY));//列印條件
            rp.SetParameterValue("ReportId", "crr020");//程式編號
