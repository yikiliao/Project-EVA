...
{
    public partial class crr007 : Form
    {
        ...
        {            
            CrystalReport_crr007 rp = new CrystalReport_crr007();            
            ...
            if (Stype=="1")
                rp.SetParameterValue("ReportName", "在職人員分析表");//報表名稱            
            if (Stype=="2")
                rp.SetParameterValue("ReportName", "離職人員分析表");//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, Beg_date, End_date, Stype));//列印條件
            rp.SetParameterValue("ReportId", "crr007");//程式編號
