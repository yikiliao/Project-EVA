...
/// <param name="DataRang">資料範圍</param>
        public wPrno(string Dept, string Cdpet, string Type)
        {
            ...
            bindingSource1.DataSource = prt016.SToDoListPrno(Dept, Cdpet, Type).ToList();
            Add_CheckBox();
        }
        ...
        public wPrno(decimal Yy, string Dept, string Cdpet, string SType)
        {
            ...
            Config.SetWindowSize(this);
            if (SType == "prt035S")
                bindingSource1.DataSource = prt035S.ToDoList_Prno(Yy, Dept, Cdpet).ToList();
            if (SType == "prt036S")
                bindingSource1.DataSource = prt036S.ToDoList_Prno(Yy, Dept, Cdpet).ToList();
            if (SType == "prt037S")
                bindingSource1.DataSource = prt037S.ToDoList_Prno(Yy, Dept, Cdpet).ToList();
            Add_CheckBox();
        }
