...
namespace HRM.Forms
{
    public partial class hrm270 : Form
    {
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            em2 = mhrm250.ToDoList(Comp, DepartmentCode, Forg, Prno, Beg_date, End_date)
                    .Where(a => !a.ClassName.Contains("假日") && !a.ClassName.Contains("節日") && !a.ClassName.Contains("休息")).ToList();
            if (Comp == "EW")
            {
                em2 = mhrm250.ToDoList(Comp, DepartmentCode, Forg, Prno, Beg_date, End_date)
                    .Where(a => !a.ClassName.Contains("假日") && !a.ClassName.Contains("節日") && !a.ClassName.Contains("休息"))
                    .Where(a => a.Cdept == "EW2100" ||
                        a.Cdept == "EW2110" ||
                        a.Cdept == "EW2111" ||
                        a.Cdept == "EW2112" ||
                        a.Cdept == "EW2113" ||
                        a.Cdept == "EW2132" ||
                        a.Cdept == "EW2170" || a.Cdept == "EW2171" || a.Cdept == "EW2172" || a.Cdept == "EW2173" || a.Cdept == "EW2180").Where(a => a.PTime1 > 0).ToList();
            }
            if (Comp == "KV")
            {
                em2 = mhrm250.ToDoList(Comp, DepartmentCode, Forg, Prno, Beg_date, End_date)
                    .Where(a => !a.ClassName.Contains("假日") && !a.ClassName.Contains("節日") && !a.ClassName.Contains("休息"))
                    .Where(a => a.Cdept == "KV2110" ||
                    a.Cdept == "KV2120" ||
                    a.Cdept == "KV2130" ||
                    a.Cdept == "KV5200" ||
                    a.Cdept == "KV5300").Where(a => a.PTime1 > 0).ToList();
            }
            ...
            {
                Print_cs_0(Comp, DepartmentCode, Forg, Prno, Beg_date, End_date);//印資料
            }
        ...
        private void Print_cs_0(string Comp, string DepartmentCode, int Forg, string Prno, DateTime Beg_date, DateTime End_date)
        {
            ...
            string ReportName = string.Format("{0}", "加班申請單");
            string ReportCond = f_Cond();
            string ReportId = string.Format("{0}", "程式編號：hrm270");


            CrystalReport_hrm270 rp = new CrystalReport_hrm270();
