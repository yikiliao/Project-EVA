...
namespace EVAERPHolday.Models
{
    class LeaveChange
    {
        ...
        public static IEnumerable<LeaveChange> ToDoList(string Leave_no)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Leave_no.Trim());
            sql = null;
            ...
            sql += " and VA_LeaveChange.leave_no =?";

            DataSet DeptDS = DBConnector.executeQuery("OA", sql, parm);
            return from p in DeptDS.Tables[0].AsEnumerable()
                   select new LeaveChange
                   {
                       Leave_no = p.IsNull("change_no") ? "" : p.Field<string>("change_no").Trim(),
                       Add_date = p.IsNull("up_date") ? "" : p.Field<string>("up_date").Trim(),
                       Applicant = p.IsNull("up_user") ? "" : p.Field<string>("up_user").Trim(),
                       Applicant_man = p.IsNull("up_user_name") ? "" : p.Field<string>("up_user_name").Trim(),
                       ...
                       Total_time = p.IsNull("ch_total_time") ? 0 : p.Field<decimal>("ch_total_time"),
                       Reason = p.IsNull("ch_reason") ? "" : p.Field<string>("ch_reason").Trim(),
