...
{
    public partial class crr012 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (mcrr012.ToDoList(YY, Dept, Cdept).Count() == 0)
            {
                Config.Show("...無符合資料...");
        ...
        {            
            CrystalReport_crr012 rp = new CrystalReport_crr012();
            rp.SetDataSource(mcrr012.ToDoList(YY, Dept, Cdept));
                        
            rp.SetParameterValue("CompName", sst011.Get(Login.DEPT).Company_name);//公司名稱
            rp.SetParameterValue("ReportName", string.Format("{0}年度員工特別休假給假標準對照表",YY ));//報表名稱
            rp.SetParameterValue("ReportCond", f_Cond(Dept, Cdept, YY));//列印條件
            rp.SetParameterValue("ReportId", "crr012");//程式編號
