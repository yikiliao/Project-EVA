...
namespace EDHR.Forms
{
    ...
    {   
        ...
        private void dept_dearch_bt_Click(object sender, EventArgs e)
        {
            ssi011w fm = new ssi011w();//廠部窗資料
            if (fm.ShowDialog() == DialogResult.OK)
            {
                if (Menu_Sel == "Qry")
                {                    
                   SetColumn(this); //解除欄位限制                   
                   Col_Disable();
                }
                f_dept.Text = fm.Company as string;
                f_dept_name.Text = fm.Company_shname as string;                
            }            
            _dg = string.Format("in ('{0}')", f_dept.Text);
        }
