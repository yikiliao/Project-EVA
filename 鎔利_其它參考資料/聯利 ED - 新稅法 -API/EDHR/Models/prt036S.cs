...
namespace EDHR.Models
{
    class prt036S
    {
        ...
        public static IEnumerable<prt036S> ToDoList(decimal Yy, string Dept, string Cdept, string Pr_no)
        {
            string sql = null;
            ...
            sql = null;
            sql += "select * from prt036S where 1= 1 ";
            sql += " and yy =?";
            sql += " and dept =?";
            //部門
        ...
        public string Delete(decimal Yy, string Dept)
        {
            try
            {
                ...
                string sql = null;
                sql += "delete from prt036S where 1=1 ";
                sql += " and yy =?";
                sql += " and dept =?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
        ...
        public static IEnumerable<prt036S> ToDoList_YY()
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            sql = null;
            sql += "select DISTINCT(yy) from prt036S ";
            sql += " order by 1 desc";
        ...
        public static IEnumerable<prt036S> ToDoList_Cdept(decimal Yy, string Dept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Dept);
            sql = null;
            sql += "select DISTINCT(cdept),prt001.department_name_new  from prt036S ";
            sql += "LEFT OUTER JOIN prt001 on prt001.department_code = prt036S.cdept";
            sql += " where 1=1 ";
            sql += " and prt036S.yy=?";
            sql += " and prt036S.dept=?";
            sql += " order by cdept";
        ...
        public static IEnumerable<prt036S> ToDoList_Prno(decimal Yy, string Dept, string Cdept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Dept);
            sql = null;
            sql += "select prt036S.pr_no pr_no,prt016.pr_name pr_name  from prt036S ";
            sql += "LEFT OUTER JOIN prt016 on prt016.pr_no = prt036S.pr_no ";
            sql += " where 1=1 ";
            sql += " and prt036S.yy=?";
            sql += " and prt036S.dept=?";
            if (!string.IsNullOrEmpty(Cdept.Trim()))
            {
                sql += " and prt036S.cdept in " + string.Format("({0})", Cdept.Trim());
            }
            sql += " order by prt036S.pr_no";
        ...
        public static prt036S Calcute_Bonus(decimal Yy, string Dept, string Cdept, string Pr_no, decimal Check_comp)
        {
            ...
            var p_prt036S = new prt036S();

            p_prt036S.Yy = Yy;
            p_prt036S.Dept = Dept;
            p_prt036S.Cdept = Cdept;
            p_prt036S.Pr_no = Pr_no;

            p_prt036S.Pay_3 = p_prt021.Pay_3;//基本薪
            p_prt036S.Pay_4 = p_prt021.Pay_4;// 職務津貼 
            p_prt036S.Pay_5 = p_prt021.Pay_5;//技術津貼
            p_prt036S.Pay_8 = p_prt021.Pay_8;//工作津貼
            p_prt036S.Pay_9 = p_prt021.Pay_9;//主管津貼
            p_prt036S.Pay = p_prt021.Pay_3 + p_prt021.Pay_4 + p_prt021.Pay_5 + p_prt021.Pay_8 + p_prt021.Pay_9; //應發薪資
            p_prt036S.Check_base = Cal_Person_Base(Yy, Pr_no); //年資基數
            p_prt036S.Check_point = prt035S.Get(Yy, Dept, Cdept, Pr_no).Tot;//考核基數
            p_prt036S.Check_comp = Check_comp; //給付標準
            //基準年終獎
            p_prt036S.Y_bonus = Math.Round(p_prt036S.Pay * p_prt036S.Check_base * p_prt036S.Check_point * p_prt036S.Check_comp, 0, MidpointRounding.AwayFromZero);

            //請假未請時數
            p_prt036S.Un_sp_hoday = Get_unHour(Yy, Pr_no);

            //特別假未請補助
            p_prt036S.Bonus_ho = (p_prt036S.Y_bonus / (22 * 12 * 8)) * p_prt036S.Un_sp_hoday;
            p_prt036S.Bonus_ho = Math.Round(p_prt036S.Bonus_ho, 0, MidpointRounding.AwayFromZero);
            ...
            if (p_prvacam == null)
                ...
                p_prt036S.Hoday = p_prvacam.Vaca_c + p_prvacam.Vaca_f + p_prvacam.Vaca_g + p_prvacam.Vaca_h + p_prvacam.Vaca_i + p_prvacam.Vaca_j + p_prvacam.Vaca_k;

            //扣減請假 4捨5入取整數 固定264(每月上班22天*12月 )
            p_prt036S.S_hoday = Math.Round(p_prt036S.Y_bonus / 264 / 8 * p_prt036S.Hoday, 0, MidpointRounding.AwayFromZero);//扣減請假 4捨5入取整數

            //應付年終獎金
            p_prt036S.T_bonus = Math.Round(p_prt036S.Y_bonus + p_prt036S.Bonus_ho - p_prt036S.S_hoday, 0, MidpointRounding.AwayFromZero);

            //主管調整增加數
            p_prt036S.S_extra = 0;

            //年度總獎金
            p_prt036S.X_bonus = Math.Round(p_prt036S.T_bonus + p_prt036S.S_extra, 0, MidpointRounding.AwayFromZero);
            ...
            //實領獎金
            p_prt036S.Bonus = Math.Round(p_prt036S.X_bonus - p_prt036S.S_tax - p_prt036S.S_bonus, 0, MidpointRounding.AwayFromZero);

            p_prt036S.Memo = string.Empty;
            ...
            return new prt036S
            {
                ...
                Cdept = Cdept,
                Pr_no = Pr_no,
                Pay_3 = p_prt036S.Pay_3,
                Pay_4 = p_prt036S.Pay_4,
                Pay_5 = p_prt036S.Pay_5,
                Pay_8 = p_prt036S.Pay_8,
                Pay_9 = p_prt036S.Pay_9,
                Pay = p_prt036S.Pay,
                Check_base = p_prt036S.Check_base,
                Check_point = p_prt036S.Check_point,
                Check_comp = p_prt036S.Check_comp,
                Y_bonus = p_prt036S.Y_bonus,
                Bonus_ho = p_prt036S.Bonus_ho,
                S_hoday = p_prt036S.S_hoday,
                T_bonus = p_prt036S.T_bonus,
                S_extra = p_prt036S.S_extra,
                X_bonus = p_prt036S.X_bonus,
                S_tax = p_prt036S.S_tax,
                S_bonus = p_prt036S.S_bonus,
                Bonus = p_prt036S.Bonus,
                Hoday = p_prt036S.Hoday,
                Un_sp_hoday = p_prt036S.Un_sp_hoday,
                Memo = p_prt036S.Memo
            };
        }
        ...
        public string Update2(decimal Yy, string Dept, string Cdept, string Pr_no)
        {
            try
            {
                ...
                string sql = null;
                sql += "update prt036S set t_bonus=?,s_extra=?,x_bonus=?,s_bonus=?,s_tax=?,bonus=? ";
                sql += " where yy=?";
                sql += " and dept=?";
                sql += " and cdept=?";
                sql += " and pr_no=?";
