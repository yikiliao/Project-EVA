...
namespace EDHR.Models
{
    class mcrr040D
    {
        ...
        public static IEnumerable<mcrr040D> ToDoList(string Cdept, Int16 Year, Int16 Month, string Type)
        {
            decimal[] qq = { 0M, 0M, 0M, 0M, 0M };
            var pp = mcrr024.ToDoList(Cdept, Year, Month, Type).ToList();
            foreach (var item in pp)
            {
                decimal Tax_amt = prt013.ToDoList("", "", "").Where(a => a.Dept == "D" && a.Nation == item.Nation && a.Vaild == "Y").ToList()[0].Tax_amt;
                qq = Get(item.Amt_16, item.Amt_17, item.Amt_21, item.Amt_23, item.Amt_26, Tax_amt, "D");
                var p_aa = prt016.ToDoList("D", item.Pr_No, "", "", "").ToList();
                yield return new mcrr040D
                {
                    Yy = Year,
                    Mm = Month,
                    Cdept = item.Cdept,
                    ...
                    School = string.IsNullOrEmpty(p_aa[0].Pr_schl) ? "" : prt006.Get("D", "SC", p_aa[0].Pr_schl).Code_desc,
                    Tax_1 = item.Tax_1,
                    Tax_2 = item.Tax_2,
                    Tax_3 = item.Tax_3,
                    Tax_4 = item.Tax_4,
