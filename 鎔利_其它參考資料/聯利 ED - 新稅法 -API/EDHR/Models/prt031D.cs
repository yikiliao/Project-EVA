...
namespace EDHR.Models
{
    class prt031D
    {
        ...
        public static IEnumerable<prt031D> ToDoList(decimal? Yy, decimal? Mm, string Pr_no, decimal? Pr_sqe)
        {
            string sql = null;
            ArrayList parm = new ArrayList();

            sql = null;
            sql += "select * from prt031D where 1= 1 ";
        ...
        public static IEnumerable<prt031D> ToDoList(decimal? Yy, decimal? Mm, string Pr_no, decimal? Pr_sqe, string Pr_cdept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
        ...
        public string Delete(decimal Yy, decimal Mm, string Cdept_no)
        {
            try
            {
                ...
                parm.Add(Cdept_no);
                string sql = null;
                sql += "delete from prt031D where yy=? and mm=? and cdept_no=? ";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
        ...
        public string Delete(decimal Yy, decimal Mm, string Pr_no, decimal Pr_sqe)
        {
            try
            {
                ...
                parm.Add(Pr_sqe);
                string sql = null;
                sql += "delete from prt031D where yy=? and mm=? and pr_no=? and pr_sqe=?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
        ...
        public string Update(decimal Yy, decimal Mm, string Pr_no, decimal Pr_sqe)
        {
            try
            {
                ...
                parm.Add(Pr_sqe);
                string sql = null;
                sql += "update prt031D set tot_wtime=?,tot_vtime1=?,tot_vtime2=?,tot_ntime=?,tot_atime1=?,tot_atime2=?,tot_atime=?,";
                sql += "amt_1=?,amt_2=?,amt_3=?,amt_4=?,amt_5=?,amt_6=?,amt_7=?,amt_8=?,amt_9=?,amt_10=?,";
                sql += "amt_11=?,amt_12=?,amt_13=?,amt_14=?,amt_15=?,amt_16=?,amt_17=?,amt_18=?,amt_19=?,amt_20=?,";
                sql += "amt_21=?,amt_22=?,amt_23=?,amt_25=?,amt_26=?,amt_27=?,";
        ...
        public static prt031D Get(decimal Yy, decimal Mm, string Pr_no, decimal Pr_sqe)
        {
            // 查詢資料庫資料
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
        ...
        public static prt031D Pay_TrnSingle(string Dept, decimal Yy, decimal Mm, string Pr_no, decimal Pr_sqe,
            decimal Tot_wtime, decimal Tot_vtime1, decimal Tot_vtime2, decimal Tot_ntime, decimal Tot_atime1, decimal Tot_atime2, decimal Tot_atime)
        {
            ...
            //遲到            
            int tot_vcode1a = prt030D.ToDoList(Dept, _yy, _mm, Pr_no).Sum(m => m.Pr_add1);
            int tot_vcode1b = prt030D.ToDoList(Dept, _yy, _mm, Pr_no).Sum(m => m.Pr_add2);


            var p_prt031D = new prt031D();
            p_prt031D.Yy = Convert.ToDecimal(_yy);
            p_prt031D.Mm = Convert.ToDecimal(_mm);
            p_prt031D.Pr_no = Pr_no;
            p_prt031D.Pr_sqe = Convert.ToDecimal(1);//要找最大值

            p_prt031D.Tot_wtime = tot_wtime;//出勤時數
            p_prt031D.Tot_vtime1 = tot_vtime1;//請假時數
            p_prt031D.Tot_vtime2 = tot_vtime2;//曠職時數
            p_prt031D.Tot_ntime = tot_ntime;//夜    班
            p_prt031D.Tot_atime1 = tot_atime1;//平日加班
            p_prt031D.Tot_atime2 = tot_atime2;//假日加班
            p_prt031D.Tot_atime = tot_atime;//總加班時數
            ...
            // 基本薪(amt1)
            p_prt031D.Amt_1 = Math.Round(p_prt021.Pay_3, 0, MidpointRounding.AwayFromZero);


            var tmp_day = p_prt031D.Tot_vtime1 / 8; //請假天數            

            //職務津貼(amt2)
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_4 / 21.75M) * tmp_day;
            p_prt031D.Amt_2 = p_prt021.Pay_4 - tmp_amt;
            p_prt031D.Amt_2 = Math.Round(p_prt031D.Amt_2, 0, MidpointRounding.AwayFromZero);
            ...
            p_prt031D.Amt_4 = p_prt021.Pay_6 - tmp_amt;
            p_prt031D.Amt_4 = Math.Round(p_prt031D.Amt_4, 0, MidpointRounding.AwayFromZero);

            //加班津貼(amt5)
            tmp_amt = 0;
            if (p_prt021.Pay_3 <= 1500)
            {
                var aa1 = (1500 / 21.75M) / 8 * 1.5M * p_prt031D.Tot_atime1;
                var aa2 = (1500 / 21.75M) / 8 * 2 * p_prt031D.Tot_atime2;
                tmp_amt = aa1 + aa2;
            }
            else
            {
                var aa1 = (p_prt021.Pay_3 / 21.75M) / 8 * 1.5M * p_prt031D.Tot_atime1;
                aa1 = Math.Round(aa1, 0, MidpointRounding.AwayFromZero);
                var aa2 = (((p_prt021.Pay_3 / 21.75M) / 8) * 2) * p_prt031D.Tot_atime2;
                aa2 = Math.Round(aa2, 0, MidpointRounding.AwayFromZero);
                tmp_amt = aa1 + aa2;
            }
            p_prt031D.Amt_5 = Math.Round(tmp_amt, 0, MidpointRounding.AwayFromZero);

            //夜班津貼(amt6)
            p_prt031D.Amt_6 = Math.Round(p_prt031D.Tot_ntime * 6, 0, MidpointRounding.AwayFromZero);

            //技術津貼(amt7)
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_5 / 21.75M) * tmp_day;
            p_prt031D.Amt_7 = p_prt021.Pay_5 - tmp_amt;
            p_prt031D.Amt_7 = Math.Round(p_prt031D.Amt_7, 0, MidpointRounding.AwayFromZero);

            //工作津貼(amt8)
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_8 / 21.75M) * tmp_day;
            p_prt031D.Amt_8 = p_prt021.Pay_8 - tmp_amt;
            p_prt031D.Amt_8 = Math.Round(p_prt031D.Amt_8, 0, MidpointRounding.AwayFromZero);

            //外勤補助(amt9)
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_11 / 21.75M) * tmp_day;
            p_prt031D.Amt_9 = p_prt021.Pay_11 - tmp_amt;
            p_prt031D.Amt_9 = Math.Round(p_prt031D.Amt_9, 0, MidpointRounding.AwayFromZero);

            //績效獎金(amt10)
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_12 / 21.75M) * tmp_day;
            p_prt031D.Amt_10 = p_prt021.Pay_12 - tmp_amt;
            p_prt031D.Amt_10 = Math.Round(p_prt031D.Amt_10, 0, MidpointRounding.AwayFromZero);

            //請假曠職(amt11)            
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_3 / 21.75M) / 8 * p_prt031D.Tot_vtime1;//請假時數
            var am1 = tmp_amt;
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_3 / 21.75M) / 8 * 3 * p_prt031D.Tot_vtime2;//曠職時數
            var am2 = tmp_amt;
            p_prt031D.Amt_11 = am1 + am2;
            p_prt031D.Amt_11 = Math.Round(p_prt031D.Amt_11, 0, MidpointRounding.AwayFromZero);

            //遲到早退(amt12)遲到時間+早退時間
            p_prt031D.Amt_12 = tot_vcode1a + tot_vcode1b;
            p_prt031D.Amt_12 = Math.Round(p_prt031D.Amt_12, 0, MidpointRounding.AwayFromZero);

            //主管津貼(amt13)
            tmp_amt = 0;
            tmp_amt = (p_prt021.Pay_9 / 21.75M) * tmp_day;
            p_prt031D.Amt_13 = p_prt021.Pay_9 - tmp_amt;
            p_prt031D.Amt_13 = Math.Round(p_prt031D.Amt_13, 0, MidpointRounding.AwayFromZero);
            ...
            //應發金額(amt16)
            p_prt031D.Amt_16 = p_prt031D.Amt_1 + p_prt031D.Amt_2 + p_prt031D.Amt_3 +
                p_prt031D.Amt_4 + p_prt031D.Amt_5 + p_prt031D.Amt_6 +
                p_prt031D.Amt_7 + p_prt031D.Amt_8 + p_prt031D.Amt_9 +
                p_prt031D.Amt_10 - p_prt031D.Amt_11 - p_prt031D.Amt_12 +
                p_prt031D.Amt_13 + p_prt031D.Amt_14 - p_prt031D.Amt_15 + p_prt031D.Amt_27;
            p_prt031D.Amt_16 = Math.Round(p_prt031D.Amt_16, 0, MidpointRounding.AwayFromZero);


            //合醫保險(amt17)            
            p_prt031D.Amt_17 = prt033.Get(p_prt016.Pr_dept, p_prt016.Medic_safe_no).Medic_sig_amt;
            p_prt031D.Amt_17 = Math.Round(p_prt031D.Amt_17, 2, MidpointRounding.AwayFromZero);
            ...
            //養老保險(amt21) 
            p_prt031D.Amt_21 = prt033.Get(p_prt016.Pr_dept, p_prt016.Old_safe_no).Old_sig_amt;
            p_prt031D.Amt_21 = Math.Round(p_prt031D.Amt_21, 2, MidpointRounding.AwayFromZero);

            //住房補貼(amt22)--外國人才有(台幹)
            p_prt031D.Amt_22 = 0;
            if (p_prt016.Nation == "1")//外籍
            {
                p_prt031D.Amt_22 = Math.Round(p_prt031D.Amt_16 * 0.13M, 2, MidpointRounding.AwayFromZero);
            }
            p_prt031D.Amt_22 = Math.Round(p_prt031D.Amt_22, 2, MidpointRounding.AwayFromZero);

            //失業保險(amt23)
            p_prt031D.Amt_23 = 0;
            p_prt031D.Amt_23 = prt033.Get(p_prt016.Pr_dept, p_prt016.Job_safe_no).Job_sig_amt;
            p_prt031D.Amt_23 = Math.Round(p_prt031D.Amt_23, 2, MidpointRounding.AwayFromZero);

            //住房公基金(amt26)公式未定
            p_prt031D.Amt_26 = 0;
            p_prt031D.Amt_26 = prt033.Get(p_prt016.Pr_dept, p_prt016.House_safe_no).House_sig_amt;
            p_prt031D.Amt_26 = Math.Round(p_prt031D.Amt_26, 2, MidpointRounding.AwayFromZero);

            //所得稅
            p_prt031D.Amt_19 = CalcuteTax.Get(Yy, Mm, Pr_no, p_prt031D.Amt_16, p_prt031D.Amt_17, p_prt031D.Amt_21, p_prt031D.Amt_22, p_prt031D.Amt_23, p_prt031D.Amt_26).Amt_20;


            //實發金額-->應發薪資-醫療保險-養老保險-失業保險-住房公積金-所得稅
            if (p_prt016.Nation == "0")//本國
            {
                p_prt031D.Amt_25 = p_prt031D.Amt_16 - p_prt031D.Amt_17 - p_prt031D.Amt_20 - p_prt031D.Amt_21 - p_prt031D.Amt_23 - p_prt031D.Amt_26 - p_prt031D.Amt_19;
            }
            else //外國
            {
                p_prt031D.Amt_25 = p_prt031D.Amt_16 - p_prt031D.Amt_19;
            }
            if (p_prt031D.Amt_25 < 0)
                p_prt031D.Amt_25 = 0;
            p_prt031D.Amt_25 = Math.Round(p_prt031D.Amt_25, 2, MidpointRounding.AwayFromZero);
        ...
        public static prt031D PAmt19(decimal Yy, decimal Mm, string Pr_no)
        {
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Mm);
            parm.Add(Pr_no.Trim());
            string sql = null;
            sql += "select sum(amt_19) amt_19 from prt031D";
            sql += " where yy =?";
            sql += " and mm >= 1";
            sql += " and mm < ?";
            sql += " and pr_no =?";
        ...
        public static prt031D PAmt19Get12(decimal Yy, decimal Mm, string Pr_no)
        {
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Mm);
        ...
        //當年度累計稅率
        public static prt031D TSumGet(decimal Yy, decimal Mm, string Pr_no)
        {
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Mm);
        ...
        public static prt031D TSumGet12(decimal Yy, decimal Mm, string Pr_no)
        {
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Mm);
