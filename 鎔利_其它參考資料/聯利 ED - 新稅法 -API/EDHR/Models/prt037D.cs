...
namespace EDHR.Models
{
    class prt037D
    {
        ...
        public static IEnumerable<prt037D> ToDoList(decimal Yy, string Dept, string Cdept, string Pr_no)
        {
            string sql = null;
            ...
            sql = null;
            sql += "select prt037D.*,prt016.pr_name from prt037D  ";
            sql += "LEFT OUTER JOIN prt016 on prt016.pr_no = prt037D.pr_no";
            sql += " where 1=1";
            sql += " and prt037D.yy =?";
            sql += " and prt037D.dept =?";
            //部門
            if (!string.IsNullOrEmpty(Cdept.Trim()))
            {
                sql += " and prt037D.cdept in " + String.Format("({0})", Cdept.Trim());
            }
            //工號
            if (!string.IsNullOrEmpty(Pr_no.Trim()))
            {
                sql += " and prt037D.pr_no in " + String.Format("({0})", Pr_no.Trim());
            }
            sql += " order by prt037D.yy,prt037D.dept,prt037D.pr_no ";
        ...
        public string Delete(decimal Yy, string Dept)
        {
            try
            {
                ...
                string sql = null;
                sql += "delete from prt037D where 1=1 ";
                sql += " and yy =?";
                sql += " and dept =?";
                if (DBConnector.executeSQL(Conn.GetStr(Login.DB), sql, parm) == 0)
                    return "刪除失敗";
        ...
        public static IEnumerable<prt037D> ToDoList_Cdept(decimal Yy, string Dept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Dept);
            sql = null;
            sql += "select DISTINCT(cdept),prt001.department_name_new  from prt037D ";
            sql += "LEFT OUTER JOIN prt001 on prt001.department_code = prt037D.cdept";
            sql += " where 1=1 ";
            sql += " and prt037D.yy=?";
            sql += " and prt037D.dept=?";
        ...
        public static IEnumerable<prt037D> ToDoList_Prno(decimal Yy, string Dept, string Cdept)
        {
            string sql = null;
            ArrayList parm = new ArrayList();
            parm.Add(Yy);
            parm.Add(Dept);
            sql = null;
            sql += "select prt037D.pr_no pr_no,prt016.pr_name pr_name  from prt037D ";
            sql += "LEFT OUTER JOIN prt016 on prt016.pr_no = prt037D.pr_no ";
            sql += " where 1=1 ";
            sql += " and prt037D.yy=?";
            sql += " and prt037D.dept=?";
