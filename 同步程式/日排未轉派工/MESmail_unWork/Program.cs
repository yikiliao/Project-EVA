...
namespace MESmail_Close
{
    static class Program
    {
        ...
        private static void f1(string Dept, string Active, string Begdate, string Enddate,string sorce,string Local)
        {
            ...
            {
                //發信件
                mailto.SendMail("mes003", Dept, set_mailbody(dt, Begdate, Enddate), Active);
            }
        }
        ...
        private static string set_mailbody(DataTable dt, string Begdate,string Enddate)
        {
            ...
            _body += String.Format("{0}<br>", "MES系統在此通知日排程未轉派工狀況如下--");
            _body += String.Format("稽核日期：{0}<br>", Begdate);
            _body += "<br>";
            _body += "<table border=1>";
            _body += string.Format("<tr style='text-align:center;'> <td>{0}</td><td>{1}</td> <td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8}</td></tr>", "客戶", "客戶訂單", "訂單", "項次", "工單", "料號", "品名", "派工數", "日排日期(起訖)");
            ...
            _body += "<br>";
            _body += string.Format("{0}", "嘉義-劉聯坤：MES系統 生產排程｜mfd007 加工生產排程(日)作業");                       
            _body += "<br>";
            _body += "<br>";
