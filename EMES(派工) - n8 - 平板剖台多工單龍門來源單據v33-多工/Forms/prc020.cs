    ...
    public partial class prc020 : Form
    {
        ...
        {            
            ...
            if (idx > 0)
            {
                ...
                dataGridView3_CellClick(dataGridView3.Rows[0].Cells[0], new DataGridViewCellEventArgs(0, 0));//點選欄位抓取資料
            }
        }
        ...
        {            
            prc020_confirm fm = new prc020_confirm();//開視窗
            fm.Msg = "是否確定輸入下列資料？\r\n\r\n" + Msg();
            if (fm.ShowDialog() == DialogResult.OK)
            {
                bool Conf = fm.isConf;
        ...
        {            
            ...
            decimal eachMach = Math.Round(WorkTime / Qty, 2); //getminWorkTime(dataGridView3, WorkTime);// 平均單位機時 每筆幾分鐘(總工時/所有產出數；含不良+bonus+良品)
            decimal operNumber = dataGridView1.Rows.Count;//操作人數
            decimal eachPerson = Math.Round(PsonTime / Qty, 2); //eachMach * operNumber; //平均單位工時

            string sql = string.Empty;


            //報工主檔資料            
            sql += In_Proc_S(DocNo, WorkTime, eachMach, operNumber);

            //員工主檔資料
            sql += In_Ede_S(DocNo);

            //員工明細檔資料
            sql += In_Edf_S(DocNo, WorkTime, PsonTime, Qty, eachMach, operNumber, eachPerson);
        ...
        private string In_Proc_S(string Doc, int WorkTime, decimal a,decimal b)
        {
            ...
            for (int i = 0; i < dataGridView3.Rows.Count; i++)
            {
                ...
                rc.Shb03 = Workday;
                rc.Shb031 = EndTime;
                rc.Shb032 = Math.Round(EachWkTime, 2) * b;//每筆工單所需耗費時間(分攤工時時間) 投入工時
                rc.Shb033 = Math.Round(EachWkTime, 2);//每筆工單所需耗費時間(分攤工時時間) 投入機時
                rc.Shb04 = Login.RID;
                ...
                rc.Shbgrup = Employee.GetCdept(Login.RID);
                rc.Shblegal = ePlant;
                rc.Shbmodu = Login.RID;
                rc.Shborig = Employee.GetCdept(Login.RID);//報工人員部門
                rc.Shboriu = Login.RID;
                rc.Shbplant = ePlant;
                rc.Shbuser = Login.RID;
        ...
        {            
            ...
            for (int i = 0; i < dataGridView3.Rows.Count; i++)
            {
                ...
                rc.Shb03 = Workday;
                rc.Shb031 = EndTime;
                rc.Shb032 = Math.Round(EachWkTime, 2);//每筆工單所需耗費時間(分攤工時時間) 投入工時
                rc.Shb033 = Math.Round(EachWkTime, 2);//每筆工單所需耗費時間(分攤工時時間) 投入機時
                rc.Shb04 = Login.RID;
                rc.Shb05 = dataGridView3.Rows[i].Cells["shb05"].Value.ToString();               //工號
                rc.Shb06 = System.Convert.ToInt16(dataGridView3.Rows[i].Cells["shb06"].Value.ToString());//工序
        ...
        private string In_Ede_S(string Doc)
        {
            ...
            InEde rc = new InEde();
            rc.Doc = Doc;
            rc.Edegrup = Employee.GetCdept(Login.RID);
            rc.Edelegal = ePlant;
            rc.Edemodu = Login.RID;
            rc.Edeorig = Employee.GetCdept(Login.RID);
            rc.Edeoriu = Login.RID;
            rc.Edeplant = ePlant;
            rc.Edeuser = Login.RID;
            rc.Add_date = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff");
            rc.Mod_date = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff");
            rc.Status = "0";
        ...
        private string In_Edf_S(string Doc, decimal Machtime, int WorkTime, decimal Qty,decimal eachMach,decimal operNumber,decimal eachPerson)
        {
            string sql = string.Empty;
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                string Begdate = bDay + " " + bHH + ":" + bMM;
                string Enddate = bDay + " " + eHH + ":" + eMM;
                sql += In_Edf_F(Doc, Machtime, Empid, WorkTime, Begdate, Enddate, Qty, Empname, eachMach, operNumber, eachPerson);
            }
            return sql;
        }
        ...
        private string In_Proc_SD(string Doc, string SourceDoc, int WorkTime, decimal a)
        {
            ...
            for (int i = 0; i < dataGridView3.Rows.Count; i++)
            {
                ...
                rc.Shblegal = ePlant;
                rc.Shbmodu = Login.RID;
                rc.Shborig = Employee.GetCdept(Login.RID);
                rc.Shboriu = Login.RID;
                rc.Shbplant = ePlant;
                rc.Shbuser = Login.RID;
                rc.Add_date = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss.fff");
        ...
        private string In_Edf_F(string Doc, decimal Machtime, string Empid, int WorkTime, string Begdate, string Enddate, decimal rQty, string Empname,decimal eachMach,decimal operNumber,decimal eachPerson)
        {
            string sql = string.Empty;
            //單件機時
            //decimal EachMach = Math.Round((Machtime / rQty), 3);
            ...
            for (int i = 0; i < dataGridView3.Rows.Count; i++)
            {
                ...
                rc.Edf08 = Qty;     //報工數量
                rc.Edf09 = Empid;   //操作員工
                rc.Edf11 = Math.Round(eachPerson, 3);
                rc.Edf12 = Math.Round(eachPerson * Qty, 3);
                //rc.Edf11 = EachWork;//實際單件工時  0.6              
                //rc.Edf12 = perWorkTime;//個人總工時
                rc.Edf14 = Math.Round(eachMach, 3);
                rc.Edf15 = Math.Round(eachMach * Qty, 3);
                //rc.Edf14 = EachMach;//實際單件機時 0.2
                //rc.Edf15 = Machtime;//機時合計 
                rc.Edflegal = ePlant;
                rc.Edfplant = ePlant;
                rc.Add_date = DateTime.Now.ToString("yyyy/MM/dd HH:mm:dd.fff");
        ...
        private string In_Shc_S(string Doc, DataTable dt)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                rc.Shcacti = "Y";
                rc.Shcuser = Login.RID;
                rc.Shcgrup = Employee.GetCdept(Login.RID);
                rc.Shcdate = DateTime.Now.ToString("yyyy/MM/dd HH:mm:dd.fff");
                rc.Shcplant = ePlant;
                rc.Shclegal = ePlant;
                rc.Shcorig = Employee.GetCdept(Login.RID);
                rc.Shcoriu = Login.RID;
                rc.Status = "0";
        ...
        private void Setting()
        {
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                string ID = Setupini.IniReadValue("LOCAL", "ID"); //工作站
                string RID = Setupini.IniReadValue("LOCAL", "RID"); //報工人員

                var p_account = new Account();
                p_account = Account.Get(ID);
        ...
        private void rm_EmpRowDel()
        {
            ...
            if (doOkCancel("確定刪除 ？") == true)
            {
                foreach (DataGridViewRow item in this.dataGridView1.SelectedRows)
                {
                    ...
                    {
                        dataGridView1.Rows[0].Selected = true;
                        dataGridView1_CellClick(dataGridView1.Rows[0].Cells[0], new DataGridViewCellEventArgs(0, 0));//點選欄位抓取資料
                    }
                }
        ...
        {
            prc020_ok fm = new prc020_ok();//"新增成功"
            if (fm.ShowDialog() == DialogResult.OK)
            {
                //do nothing
            }
        ...
        {
            bool rf = false;
            prc020_okcanel fm = new prc020_okcanel();
            fm.Msg = Meg;
            if (fm.ShowDialog() == DialogResult.OK)
            {
                rf = fm.isConf;
        ...
        {
            if (string.IsNullOrEmpty(f_wkno.Text)) return;
            rHistory fm = new rHistory();
            fm.Dept = Login.DeptOne;//部門
            fm.Sfb01 = f_wkno.Text;//工單            
            fm.Shb06 = f_wkseq.Text;//工序
            if (fm.ShowDialog() == DialogResult.OK)
            {
                //do nothing
            }
