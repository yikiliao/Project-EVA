...
namespace MFPD.Models
{
    class mfplap
    {
        ...
        public static IEnumerable<mfplap> TToDoList(string rPlant, string rMfdate, string rMfline, string rMftable)
        {
            string sql = null;
            ...
            sql = null;
            sql += "select * from mfplap where 1=1 ";
        ...
        public static IEnumerable<mfplap> ToDoList(string rPlant, string rMfline, string rMftable, string rBegDate, string rEndDate)
        {
            string sql = null;
            ArrayList parm = new ArrayList();

            sql = null;
            sql += "select mfplap.* from mfplap ";
            sql += "where 1=1 ";
            if (!string.IsNullOrEmpty(rPlant.Trim()))
            {
                parm.Add(rPlant.Trim());
                sql += " and mfplap.plant= ?";
            }
            //生產線別
            if (!string.IsNullOrEmpty(rMfline.Trim()) && rMfline != null)
            {
                parm.Add(rMfline);
                sql += " and mfplap.mfline = ? ";
            }
            ...
            sql += " order by mfplap.plant,mfplap.mfdate,mfplap.mfline,mfplap.mftable,mfplap.wkno ";
        ...
        public static IEnumerable<mfplap> ToDoList1(string rPlant, string rMfline, string rMftable, string rBegDate, string rEndDate)
        {
            string sql = null;
            ArrayList parm = new ArrayList();

            sql = null;
            sql += "select mfplap.*,mfplah.ta01,mfplah.ta02,mfplah.ta03,mfplah.ta04,mfplah.ta05,mfplah.ta06 from mfplap ";
            sql += "LEFT OUTER JOIN mfplah on mfplap.plant = mfplah.plant and mfplap.mfdate = mfplah.mfdate and mfplap.mfline = mfplah.mfline ";
            sql += "where 1=1 ";
            if (!string.IsNullOrEmpty(rPlant.Trim()))
            {
                parm.Add(rPlant.Trim());
        ...
        public static IEnumerable<mfplap> ToDoListGroup(string rPlant, string rMfline, string rMftable, string rBegDate, string rEndDate)
        {
            string sql = null;
            ArrayList parm = new ArrayList();

            sql = null;
            sql += "select plant,mfdate,mfline from mfplap where 1= 1 ";
            if (!string.IsNullOrEmpty(rPlant.Trim()))
            {
                parm.Add(rPlant.Trim());
                sql += " and plant= ?";
        ...
        public string Delete(string Plant, string Mfdate, Int32 Seqno)
        {
            try
            {
                ...
                parm.Add(Seqno);
                string sql = null;
                sql += "delete from mfplap where plant=? and mfdate=? and seqno=? ";
                if (DBConnector.executeSQL(Login.MFPD, sql, parm) == 0)
                    return "刪除失敗";
            }
            catch (Exception ex)
        ...
        public string Update(string Plant, string Mfdate, Int32 Seqno)
        {
            try
            {
                ...
                parm.Add(Seqno);
                string sql = null;
                sql += "update mfplap set mfline=?, mftable=?,wkno=?,tubno=?,workqty=?,ta_sfb02=?,ta_sfb03=?,ta_sfb04=?,ta_sfb06=?,ta_sfb07=?,ta_sfb08=?,sp=?,cond=?,";
        ...
        public static mfplap Get(string Plant, string Mfdate, Int32 Seqno)
        {
            // 查詢資料庫資料
            ArrayList parm = new ArrayList();
        ...
        public static Int32 GetSeqNo(string Plant, string Mfdate)
        {
            ...
            string sql = "";
            sql += "SELECT max(seqno+1) aa from mfplap ";
            sql += " where plant =?";
            sql += " and mfdate = ?";
