...
{
    public partial class mfd001a : Form
    {        
        string Menu_Sel;
        ...
        public mfd001a(string Sel, string rPlant, string rMfdate, string rMfline, Int32 rSeqno)
        {
            InitializeComponent();
            FindControl(this);
        ...
        private void Show_Form(string rPlant, string rMfdate, Int32 rSeqno)
        {
            ...
            //工作桶數
            f_tubno.Value = p_mfplan.Tubno;
            f_tubno_1.Value = p_mfplan.Tubno_1;
            f_tubno_2.Value = p_mfplan.Tubno_2;
            f_tubno_3.Value = p_mfplan.Tubno_3;
            f_tubno_4.Value = p_mfplan.Tubno_4;
        ...
        private string f_Insert_1()
        {
            ...
            p_mfplan.Tubno_1 = f_tubno_1.Value;
            p_mfplan.Tubno_2 = f_tubno_2.Value;
            p_mfplan.Tubno_3 = f_tubno_3.Value;
            p_mfplan.Tubno_4 = f_tubno_4.Value;
            ...
            return "預計排程" + p_mfplan.Insert();
        }

        private string f_Insert_2()
        {
            var iPlant = f_dept.SelectedValue.ToString();
            var iMfdate = f_mfdate.Text;
            var iMfline = f_mfline.SelectedValue.ToString();
            var iMftable = f_mftable.SelectedValue.ToString();
            var iWkno = f_wkno.Text;
            
            //實際生產有資料不新增
            //if (mfplap.ToDoList(iPlant, iMfline, iMftable, iMfdate, iMfdate, iWkno).Count() > 0)
            if (mfplap.ToDoList(iPlant, iMfline, iMftable, iMfdate, iMfdate).Where(a=>a.Wkno==iWkno).Count() > 0)
            {
                return string.Empty;
            }
            else
            {
                ...
                p_mfplap.Seqno = mfplan.GetSeqNo(iPlant, iMfdate);
        ...
        {
            var p_mfplan = new mfplan();
            p_mfplan.Mfline = f_mfline.SelectedValue.ToString();
            p_mfplan.Mftable = f_mftable.SelectedValue.ToString();
            p_mfplan.Wkno = f_wkno.Text;
            ...
            return p_mfplan.Update(dPlant, dMfdate, dSeqno);
        }


        private string f_Delete()
        {
            return String.Format("{0}", f_Delete_prt021());
        }

        private string f_Delete_prt021()
        {   
            return new mfplan().Delete(dPlant, dMfdate, dSeqno);
        }
        ...
        private void Find_p(string Wkno)
        {
            ...
            //-----------------------------
            f_tubno_1.Value = p_sfb_file.Ta_sfb19 - mfplan.TToDoList(f_dept.SelectedValue.ToString(), "", "", "").Where(m => m.Wkno == Wkno).Sum(m => m.Tubno);   // 工作桶數 = 預計生產桶數-歷史合計工作桶數
        ...
        private void History_p(string Wkno)
        {
            ...
            //string gMfline = f_mfline.SelectedValue.ToString();
            //string gMftable = f_mftable.SelectedValue.ToString();
            string gWkno = Wkno;

            var _p = mfplan.TToDoList(gPlant, "", "", "").Where(m => m.Wkno == gWkno).ToList();
            bindingSource1.Clear();
            foreach (var i in _p.ToList())
            {
                mfplan a_mfplan = new mfplan();              

                a_mfplan.Mfdate = i.Mfdate;
                a_mfplan.Mfline = i.Mfline;
