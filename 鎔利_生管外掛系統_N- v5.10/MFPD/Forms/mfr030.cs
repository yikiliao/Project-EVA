...
{
    public partial class mfr030 : Form
    {        
        ...
        private void init_Form()
        {
            ...
            f_begdate.Value = DateTime.Now.AddDays(-30);//
            f_enddate.Value = DateTime.Now.AddDays(60);//
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //DataTable dt = Mfr009.unToDoList(rPlant);
            DataTable dt = Mfr009.unToDoList(rPlant, rBegdate, rEnddate);
        ...
        {
            var rpname = "未排程加工工單報表";
            CrystalReport_mfr009 rp = new CrystalReport_mfr009();
            rp.SetDataSource(dt);            
            ...
            rp.SetParameterValue("ReportId", "mfr030");//程式編號            
            rp.SetParameterValue("ReportAuthor",Login.IDNAME);//製表人
            crystalReportViewer1.ReportSource = rp;
            crystalReportViewer1.Refresh();
        }

        private string f_Cond(string rPlant, string rBegdate, string rEnddate)
        {
            string cond = "";
            ...
            {
                cond += string.Format(" | 客戶交期(起)>={0}", rBegdate);
            }
            if (!string.IsNullOrWhiteSpace(rEnddate))
            {
                cond += string.Format(" | 客戶交期(迄)<={0}", rEnddate);
            }
            if (cond.Length == 0)
            {
                cond = "印全部資料";
