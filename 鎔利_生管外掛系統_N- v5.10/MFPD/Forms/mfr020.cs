...
{
    public partial class mfr020 : Form
    {        
        ...
        private void init_Form()
        {
            ...
            f_enddate.Value = DateTime.Now.AddDays(7);//內定今天
            Set_Print_Style();
        }
        ...
        private void sty_List()
        {
            ...
            DataTable dt = ProcscaDay.ToDoList(rPlant, rBegdate, rEnddate);
            ...
            else
            {
                ...
                Print_cs_List(dt, ReportCond);
            }
            UnCursor_wait(Org_Color); //改變滑鼠游標還原預設
        }

        private void Print_cs_List(DataTable dt, string ReportCond)
        {
            var rpname = "加工日排程表(生管)";
            CrystalReport_mfr020 rp = new CrystalReport_mfr020();
            rp.SetDataSource(dt);
            ...
            rp.SetParameterValue("ReportId", "mfr020");//程式編號            
        ...
        private void sty_Sale()
        {
            ...
            DataTable dt = WorkSchDay.ToList(rPlant, string.Empty, rBegdate, rEnddate);
            var count = dt.Rows.Count;
        ...
        {
            var rpname = "加工生產排程表(日)--業務";
            CrystalReport_mfr007_2 rp = new CrystalReport_mfr007_2();
            rp.SetDataSource(dt);
            rp.SetParameterValue("CompName", Login.COMPNAME);//公司名稱            
            rp.SetParameterValue("ReportName", rpname);//報表名稱
            rp.SetParameterValue("ReportCond", ReportCond);//列印條件
        ...
        {
            var rpname = "加工生產排程表(日)--現場";
            CrystalReport_mfr007 rp = new CrystalReport_mfr007();
            rp.SetDataSource(dt);
            rp.SetParameterValue("CompName", Login.COMPNAME);//公司名稱            
            rp.SetParameterValue("ReportName", rpname);//報表名稱
            rp.SetParameterValue("ReportCond", ReportCond);//列印條件
        ...
        private void sty_Sale2()
        {
            ...
            {
                string ReportCond = f_Cond(rPlant, rBegdate, rEnddate, rPrintType);//列印條件
                Print_cs_Sale2(dt, ReportCond);
            }
            UnCursor_wait(Org_Color); //改變滑鼠游標還原預設
        }

        private void Print_cs_Sale2(DataTable dt, string ReportCond)
        {
            var rpname = "加工生產排程表(日)--工作站負荷";
            CrystalReport_mfr007_3 rp = new CrystalReport_mfr007_3();
            rp.SetDataSource(dt);
            rp.SetParameterValue("CompName", Login.COMPNAME);//公司名稱            
            rp.SetParameterValue("ReportName", rpname);//報表名稱
            rp.SetParameterValue("ReportCond", ReportCond);//列印條件
